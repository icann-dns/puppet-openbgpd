  ...
  on_supported_os.each do |os, facts|
    context "on #{os}" do
      ...
      describe 'check default config' do
        ...
        it do
          is_expected.to contain_service("openbgpd")
              .with({
                "ensure" => true,
                "enable" => true,
                })
      ...
      describe 'Change Defaults' do
        ...
        context 'router_id' do
          ...
          it do
            is_expected.to contain_file("/usr/local/etc/bgpd.conf")
              .with_content( /router-id 192.0.2.3/)
          end
        end
        context 'networks4' do
          ...
          it do
            is_expected.to contain_file("/usr/local/etc/bgpd.conf")
              .without_content( /network 192.0.2.0\/25/)
          end
        end
        context 'networks6' do
          ...
          it do
            is_expected.to contain_file("/usr/local/etc/bgpd.conf")
              .without_content( /network 2001:DB8::\/48/)
          end
        end
        context 'failsafe_networks4' do
          ...
          it do
            is_expected.to contain_file("/usr/local/etc/bgpd.conf")
              .without_content( /network 192.0.2.0\/24/)
          end
        end
        context 'failsafe_networks6' do
          ...
          it do
            is_expected.to contain_file("/usr/local/etc/bgpd.conf")
              .without_content( /network 2001:DB8::\/32/)
          end
        end
      ...
      describe 'check bad type' do
        ...
        context 'networks6 bad cidr size' do
          before { params.merge!( networks6: [
                                 '2001:DB8::/48', '2001:DB8::/1111']) }
          it { expect { subject.call }.to raise_error(Puppet::Error) }
        end
        context 'networks6 bad network' do
          before { params.merge!( networks6: [
                                 '2001:DB8::/48', '20011:DB8::/128']) }
          it { expect { subject.call }.to raise_error(Puppet::Error) }
        end
        context 'failsafe_networks4' do
